...
**Draft v3 — 2026-08-15** (supersedes v2)
**Authors:** [author list pending]
**Target venue:** RA-L / IROS 2027 (failure-analysis + measurement paper)
**Companion files:** artifacts in `artifacts/` (actions 01–06), replication runs 08/08b, matcher-variant runs in `artifacts/gt_variant_*` and `artifacts/action2v_*`. LaTeX from v2 (`latex/paper.tex`) NOT yet synced to v3 — v3 is the authoritative text.
**Version note (v3):** addresses the three structural reviewer risks identified against v2. (1) _One matcher:_ the alias signature (Sec. 4.7) and the rejection-rate structure (Sec. 4.7) are now reproduced under ORB-only, SIFT-only, and SuperPoint+LightGlue. (2) _Core class on one region:_ the boundary map is now exhaustive — all 11 regions of the source dataset classified (Sec. 4.5), plus split-half internal replication (Sec. 4.1). (3) _Negative-result core:_ a constructive deployment section (Sec. 5.1) states what works now and what would fix the sub-tile class, with the oracle bounds as measured design targets. Also added: camera-attitude and imagery-vintage confounds ruled out from dataset metadata (Sec. 4.1, Result 6); per-region ceiling-class forensics (Sec. 4.5); the field's standard fixes measured — PnP+DSM, GNC, robust smoothing (Sec. 4.8). All v2 corrections retained.
...
of the terrain, not of one pipeline. A boundary map covering all eleven
regions of the dataset shows the sub-tile class occupies exactly the one
region that is both solvable and repetitive furrow farmland; an executed
replication on an independently collected urban dataset confirms the class
is terrain-specific and exposes a 16.5 m georeferencing bias that the
diagnostic itself detects. We contribute (i) a taxonomy with per-class
rejection behaviour, (ii) the first backwards-rate tables for published
rejectors, (iii) a sign-folding diagnostic for error reporting, (iv) a
falsified countermeasure with quantified oracle bounds (median 31.1 → 14.0 m
under the strongest available oracle) that double as design targets, and
(v) a pre-registered replication protocol.
...
any one matcher, and we quantify what would be required to fix it.
...
3. **Matcher independence**: the alias signature and the rate structure
   ...
   learned detector–matcher (Sec. 4.7).
...
the dataset does not publish satellite capture dates.
...
boundary is a threshold choice, and the strict side is the safe one. The
sub-tile class is therefore doubly specific: it requires not only
repetitive solvable terrain but _planar_ repetitive terrain — which is why
the only furrow farmland in the dataset is also the only place the class
appears.
...
5. **Expect the ceiling class to be geometry, not texture.** Six of
   eleven regions fail the planar-homography assumption with abundant
   features (16–57 correspondences, 9–30% geometric conversion, Sec. 4.5):
   on non-planar terrain route around the flat matcher (PnP+DSM-class
   methods or a different anchor), and check reference-tile coverage —
   the R09 gap (14/40 GT tiles absent) shows coverage checks belong in
   the pipeline.
...
regime extracts nothing (31.2 m); a tighter prior alone cannot help, so
odometry quality must come from elsewhere in the stack. (c) _RTK-grade
truth_ to establish whether the ~13 m R03 floor is the dataset's own GPS
noise — a prerequisite for claiming any smaller residual. These are design
targets with quantified distances, not open wishes.
...
truth-correlation guard that any future replication must pass.
...
against a 31.1 m production baseline (n=610). The class is terrain-specific
in an exhaustively mapped sense: absent on desert, clean farmland, all
ceiling-class regions, and an independent urban dataset, and present
exactly where terrain is repetitive, solvable farmland. The paper
contributes the taxonomy, the matcher-stable rate tables, the sign-folding
diagnostic, the quantified boundary of what map matching can and cannot
recover — design targets for per-field map annotation and odometry-quality
regimes we quantify as unreachable at 7 s frame spacing — and a
...
measurement + sign-folding diagnostic + falsified countermeasure + oracle
bound + exhaustive boundary map + replication protocol. Novel vs. Lajoie
(theory, indoor) and the vineyard robotics literature (qualitative,
ground): cross-view measurements, rates, appearance-optimality unanimity,
matcher replication, and the first documented replication attempts for the
class. The v2 weakness "all rates on one matcher" is closed for classical
matchers and bounded honestly for the learned one; "one region" is
converted from an unexamined selection into a measured boundary.
...
the constructive half of the paper (Sec. 5.1) is now explicit rather than
implicit in the rate tables.
...
matcher-variant streams isolated under a cross-quote rule.
...
second/third/fourth matcher measured. Missing: RTK audit of UAV-VisLoc GT;
video-rate odometry evaluation; rate-grade learned-matcher table (blocked
by its 2-fatal yield at the paper's acceptance settings). All stated as
limitations.
...
| Loose thresholds turn no-fix into wrong-fix | ratio 0.85 + thr 15 px: R02/R08/R09 "solve" 4–5/15 at 107–467 m median error                                                        | supported |
...
25. Lowe, "Distinctive Image Features from Scale-Invariant Keypoints," IJCV 2004, DOI 10.1023/B:VISI.0000029664.99615.94.
26. Rublee, Rabaud, Konolige, Bradski, "ORB: An Efficient Alternative to SIFT or SURF," ICCV 2011, DOI 10.1109/ICCV.2011.6126544.
27. DeTone, Malisiewicz, Rabinovich, "SuperPoint: Self-Supervised Interest Point Detection and Description," CVPR Workshops 2018, arXiv:1712.06117.
28. Sarlin, "LightGlue: Local Feature Matching at Light Speed," arXiv:2306.13614.
