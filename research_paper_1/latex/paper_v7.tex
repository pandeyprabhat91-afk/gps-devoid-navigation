...
\usepackage{cite}
\usepackage{url}
\setlength{\emergencystretch}{2.5em} % calm underfulls in dense two-column text
...
\markboth{Draft v7}{COHERENT-OFFSET ALIASING IN UAV-TO-SATELLITE GEO-LOCALIZATION}

\maketitle

\begin{abstract}
UAV geo-localization against satellite reference imagery fails on repetitive terrain in a way standard robustness tools do not visibly catch. We observe that wrong matches can form spatially coherent modes: the estimate sits tens to hundreds of metres from truth, yet consecutive wrong estimates agree with each other. A two-line model explains the consequence: if the estimate equals truth plus a persistent offset, frame-to-frame relative motion is preserved, so temporal-consistency checks observe nothing wrong. We measure the effect on UAV-VisLoc (6,742 images, 11 regions), separating whole-tile aliases (a look-alike tile hundreds of metres away) from sub-tile aliases (the correct tile, displaced by one period of a repeating structure). Signed per-frame offsets ($n=610$) reveal bimodality with a hole at zero that magnitude histograms hide. Sequential consistency and a three-keyframe rule retain wrong fixes at higher rates than correct ones on the sub-tile class, while pairwise and alignment methods discriminate only by discarding most good fixes. The signature reproduces across four matcher families; a dense matcher adds its own confidently-wrong warp mode. A falsified mixture countermeasure and a 14.0\,m oracle bound quantify the residual. Frame spacing governs operability: identical propagation chains deliver 97\% coverage at metre spacing and collapse at 250\,m stride. A boundary map over all eleven regions, an independent-dataset replication, and a pre-registered replication protocol complete the contribution.
\end{abstract}
...
\IEEEPARstart{G}{PS}-denied UAV navigation increasingly relies on matching nadir drone imagery against georeferenced satellite tiles to anchor drift-prone visual-inertial odometry. Published systems report median errors of metres on favourable terrain~\cite{anyvisloc,orthotrack,ortholoc}. Failure analysis is thin: benchmarks report accuracy at thresholds over pooled terrain, which is blind to how, where, and why the tail fails.

The failure we document is specific: on repetitive terrain, accepted matches can be wrong by tens to hundreds of metres \emph{with full geometric and appearance support}, and consecutive wrong matches can agree with each other. A wrong match of this kind is dangerous precisely because it looks plausible---many inliers, tight fit, high similarity---and because agreement across frames is exactly what rejection tools check.

Temporal consistency and geometric consistency checks assume wrong solutions behave differently from the true trajectory. If the wrong solution carries approximately the same offset frame to frame, relative motion still looks correct, and consistency alone may not expose the error. The rest of this paper tests that intuition as a measurement study: not a new localizer, but documentation of when and why the standard toolbox cannot see the failure.


\noindent\textbf{Contributions.}
\begin{enumerate}
\item A whole-tile/sub-tile taxonomy with distinct geometry and behaviour (Sec.~\ref{sec:model}, \ref{sec:structure}).
\item Backwards-rate tables for four published rejection methods over a shared fix stream (Sec.~\ref{sec:bench}).
\item Matcher independence across four families plus a dense-matcher failure mode (Sec.~\ref{sec:matcher}).
\item The sign-folding diagnostic, a falsified countermeasure with oracle bound, an averaging bound with its exception, and a spacing-isolation measurement (Sec.~\ref{sec:structure}, \ref{sec:counter}, Appendix).
\item A terrain boundary map, an independent replication with a documented false-positive mode, and a pre-registered replication protocol (Sec.~\ref{sec:boundary}, \ref{sec:replication}).
\end{enumerate}

\section{Related Work}

{\sloppy
Retrieval-based systems~\cite{uavvisloc,anyvisloc,denseuav} and geometric pipelines (homography, as in the production matcher studied here; PnP against DSM~\cite{ortholoc,orthotrack}) define \textbf{UAV--satellite geo-localization}, benchmarked by pooled accuracy-at-threshold. AnyVisLoc's Table~6 is representative: its baseline scores 74.1\% A@5\,m against an aerial photogrammetry reference but 18.5\% A@5\,m against a satellite reference~\cite{anyvisloc}---the satellite-reference tail exists, is large, and is not analyzed per-region. Our work is complementary: we analyze \emph{why} that tail exists rather than improving its head.\par}
...
A UAV frame $I_t$ is matched against a satellite reference map $M$, producing an estimate $\hat{p}_t$ of the true position $p_t$. We record the error as the signed offset $e_t = \hat{p}_t - p_t$ in (north, east): Sec.~\ref{sec:structure} shows magnitude $|e_t|$ folds away the structure the signed form reveals.

The coherent-offset model is two lines. Suppose
\begin{equation}
\hat{p}_t = p_t + c
\end{equation}
...
\end{equation}
Relative motion is preserved exactly. Any check comparing consecutive relative motions observes agreement between truth and alias alike. Two observed instances: a \emph{whole-tile} alias (a similar-looking, geographically different tile; hundreds of metres) and a \emph{sub-tile} alias (the correct tile, displaced by one structure period; tens of metres).

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/v7_f1_concept.png}
\caption{The coherent-offset model. The aliased lock $\hat{p}_t = p_t + c$ trails truth by a persistent offset $c$; frame-to-frame deltas agree ($\Delta\hat{p}=\Delta p$), so consistency checks pass on the wrong trajectory.}
\label{fig:concept}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/v7_f2_taxonomy.png}
\caption{The two alias classes (illustrative geometry). (a)~Whole-tile: the lock lands on a look-alike tile $\sim$350\,m away. (b)~Sub-tile: the lock sits on the correct tile, offset $\sim$52\,m along the furrow direction.}
\label{fig:taxonomy}
\end{figure*}
...
{\sloppy
The pipeline is a production map matcher combining pooled correspondences from ORB, AKAZE and SIFT, MAGSAC homography~\cite{magsac}, image-centre projection through the inverse homography, patch-wide NCC verification at 0.30 with the inlier floor at 10, DEM-corrected AGL scale. Candidate generation is prior-centred ($5\times5$ tile ring). Frames carry a drifted prior injected as a random walk at 150/300/600\,m. Dataset: UAV-VisLoc~\cite{uavvisloc}, eleven Chinese sites; all measurements use the dataset's drone GPS as truth.\par}
...
\centering
\footnotesize
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{2.55in}@{}}
\toprule
Item & Setting \\
\midrule
Dataset & UAV-VisLoc: 6,742 nadir images, 11 satellite regions, drone GPS truth \\
Pipeline & ORB+AKAZE+SIFT pool, MAGSAC, centre projection, NCC 0.30, inliers $\geq$10, AGL scale \\
Priors & random-walk drifted priors at 150/300/600\,m RMS (never truth) \\
Error cats & good $<$20\,m, mid 20--50\,m, fatal $\geq$50\,m (fixed a priori) \\
Metrics & position error, signed (N,E) offset, good/fatal-kept rates, coverage \\
\bottomrule
\end{tabular}
\end{table}

% Pipeline schematic moved to the Supplement (Fig.~S1) to keep the main narrative on the mechanism.
...
The six observations read as one argument: coherent modes exist (\ref{sec:structure}), consistency cannot see them (\ref{sec:bench}, \ref{sec:coherence}), recovery fails for a measured reason (\ref{sec:counter}), the class is terrain-specific (\ref{sec:boundary}), better matching preserves it (\ref{sec:matcher}), and spacing decides whether it matters operationally (\ref{sec:spacing}). Standard fixes and deployment quantifications are in the Appendix.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/v7_f3_signed.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/v7_f6_terrain.png}
\caption{The smoking-gun frame (R04, 04\_0487). (a)~Drone view at 543\,m AGL. (b)~Satellite reference crop: the accepted lock (279 inliers, NCC 0.66) sits 51.7\,m from GPS truth along the field structure.}
\label{fig:terrain}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/v7_f5_rejector.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/v7_f4_coherence.png}
...
% Boundary table moved to Appendix Table A-1.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/v7_f7_matcher.png}
\caption{R04 median error is matcher-independent across classical and learned families (LightGlue cells underpowered, reported as such).}
\label{fig:matcher}
\end{figure}
...
\midrule
pooled & 610 (83\%) & 0.31 & 167.2$^{\circ}$ & 0.18 & 31.1\,m & 2.55$\times$ ($n{=}25$) \\
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Rejector & \shortstack{pooled\\(27g/7f)} & \shortstack{ORB\\(22g/7f)} & \shortstack{SIFT\\(23g/8f)} & \shortstack{LG\\(18g/2f)} \\
\midrule
...
% Dense table moved to Appendix Table A-2.
...
Four published-style remedies (PnP against elevation, graduated robustness, a robust fixed-lag smoother, plus retrieval tuning, detector fine-tuning, per-terrain thresholds and variance weighting) were implemented with pre-registered kill criteria and all failed on the alias classes; full text and kill criteria are in Appendix Sec.~\ref{sec:app-fixes}--\ref{sec:app-negatives}.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/v7_f8_spacing.png}
\caption{Stride governs availability: per-frame baseline, dense propagation (3.8\,m), and coarse propagation (250\,m) on 429 video-rate frames.}
\label{fig:spacing}
\end{figure}
...
% Matched-modal table moved to Appendix Table A-3.
...
Fix averaging over accepted-fix streams is bounded in Appendix Sec.~\ref{sec:app-deploy}: ten-match averaging cuts the pooled median 21.8 to 8.0\,m but first hurts on coherent terrain (R06 at $k{=}5$ nearly doubles $k{=}1$).
...
\item \textbf{Correct AGL scale, expect geometry ceilings, and know where difficulty lies.} Four deployment quantifications (AGL correction effect, ceiling-class geometry evidence, difficulty decomposition, estimator receipt) are in Appendix Sec.~\ref{sec:app-deploy}.
\end{enumerate}
...
(i)~Pooled A@$X$\,m metrics on repetitive terrain hide a failure class with full geometric support; signed-offset reporting is a cheap diagnostic that exposes it. (ii)~Robust-estimation defaults do not transfer unexamined to cross-view geo-localization on repetitive terrain: they fail backwards or at coverage-destroying purity, at measured rates, under every matcher tested. (iii)~The fix that works for one alias class does not exist for the other; hedging is safe but inert, mode-commitment is dangerous (68.4\,m at $\sigma=300$). (iv)~An independent replication attempt shows the diagnostic's false-positive mode and a truth-correlation guard that any future replication must pass. (v)~Dense matcher benchmarks transfer only within a modal-proximity regime. (vi)~Frame spacing is a first-class experimental variable: the same chain that collapses at 250\,m stride delivers 97\% frame coverage at metre spacing. (vii)~On generality, stated carefully: the \emph{mechanism} (persistent offsets defeat consistency checks) is general mathematics; its \emph{occurrence rate} is demonstrated on the tested regions, reproduced across the tested matchers, and not established beyond them.

\subsection{What should a future system do?}
Gate what is separable with calibrated prior uncertainty; survive what is not through adaptive weighting and odometry coast; report signed errors so the next alias class is visible; match the capture rate to the continuity the filter assumes; and treat per-field map annotation as the only identified route to the sub-tile residual.
...
Wrong matches in UAV-to-satellite geo-localization can be spatially coherent: stable offsets of tens to hundreds of metres with full geometric and appearance support.

Because a persistent offset preserves relative motion exactly, temporal-consistency rejection observes agreement where it should observe disagreement---and does, backwards, at measured rates on the sub-tile class.

The class is terrain-specific, matcher-independent in signature, partly separable with calibrated priors (whole-tile) and unrecoverable per-frame otherwise (sub-tile, 14.0\,m oracle residual). Frame spacing decides whether the failure matters operationally: metre spacing contains it through propagation, 250\,m stride does not.

Design outward from these measurements: report signed errors, gate what is separable, survive what is not, and capture at the rate the filter assumes.

\appendix
\section{Supplementary experiments}

\subsection{Field-standard fixes, measured in full}
\label{sec:app-fixes}
\label{sec:fixes}

\textbf{PnP against the DSM on the ceiling class---killed.} {\sloppy
Lifting GT-tile correspondences to DEM elevation (90\,m grid) in the AnyVisLoc/OrthoLoC/OrthoTrack geometry and solving PnP-RANSAC on the seven ceiling regions solves \emph{fewer} frames than the production homography everywhere (healthy control R03: 32 vs.\ 12 of 40) and worse where it solves (median 34--338\,m vs.\ homography's 11.6--37.6\,m). The ceiling class is correspondence-limited, not model-limited.\par}
...
\subsection{Retrieval, tuning and weighting negatives}
\label{sec:app-negatives}
The remedies paragraph of Sec.~4.8 in full: zero-shot retrieval floor absent (GT rank 53--1556), homographic fine-tune bottleneck in the matcher, per-terrain NCC adoption killed by an added fatal, DEM-variance weighting $r=-0.21$, moment pre-gate AUC 0.52. All pre-registered, all killed.

\subsection{Deployment quantifications}
...
\begin{table}[h]
\caption{Terrain boundary of the alias classes (from Sec.~4.5).}
\label{tab:terrain-app}
\centering
\footnotesize
Boundary table as measured (see artifact \texttt{boundary\_map.json}):
R01/R02/R05/R07/R08/R09/R10 unsolvable-class (geometry: non-planar, relief, canopy, missing tiles);
R03/R11 clean controls; R06 whole-tile; R04 sub-tile. Full per-region rows:
\end{table}
\begin{table*}[t]
\caption{Terrain boundary counts (detail). Solved = GT-tile probe ($n=40$ step-sampled unless stated). Corr med $\to$ inlier med = median correspondence count $\to$ median MAGSAC inliers (conversion \%).}
\label{tab:terrain}
\centering
\footnotesize
\begin{tabular}{@{}lllll@{}}
...
Five steps with exact commands and pre-registered kill criteria (artifacts and scripts accompany the paper): sign-folding diagnostic, backwards-rate benchmark, coherence curve, countermeasure gate, boundary probe. AerialVL executed (class absent; guard fired); TEMPO-VINE rejected on structure; ViLD pending access.
